...
манифест вшивается в APK (`build.sh` копирует его в `assets/models_manifest.json` и падает, если
`app` в нём не равен версии приложения или файлы из репозитория разошлись с хэшами). После смены
любого файла: пересчитать `size`/`sha256`, поднять `app`, выложить `tools/models_publish.sh`.
Стендовый источник вместо Hugging Face — `tools/models_serve.py` (Range, обрывы, битые файлы, 404).
